...
async def run_super(rockets):
    rocket_split = [rockets[0 + 100 * q:100 + 100 * q] for q in range(0, 1000)]
    threads = list()
    for rocket_pack in rocket_split:
        threads.append(Thread(target= await run_async(rocket_pack)))
...
def main():
    ...
    #run_super(async_rockets)

    # run_sync_one_thread(rockets)
    # run_in_threads(rockets)
    asyncio.run(run_super(async_rockets))
